...
class OrderService
{
    ...
    public function page(){
    	...
    	//封装where查询条件
        empty($data['uid'])			|| $where['uid'] 		= 	$data['uid'];
    	(empty($data['status']) 	|| $data['status'] == '')	|| $where['status'] 	= 	$data['status'];
    	empty($data['trade_no'])	|| $where['trade_no']		= 	['like','%'.$data['trade_no'] ];
    	empty($data['out_trade_no'])	|| $where['out_trade_no']		= 	['like','%'.$data['out_trade_no'] ];

        return Order::where($where)->order('create_time desc')->whereTime('create_time','month')->paginate(15);
    }

    public function getNew(){
        return Order::order('order_id desc')->paginate(10);
    }
